...
class MemeCreator:
    ...
    def manual_workflow(self):
        workflow = [
            {
                "action_type": "chat",
                "action": "Clarify what kind of meme the user wants and refine the meme description in English.",
                "tool_use": []
            },
            {
                "action_type": "tool_use",  # 这里标记为工具步骤
                "action": "Generate a meme image based on the refined description using the text-to-image tool.",
                "tool_use": ["text_to_image"]  # 这里的名字要和 tool['function']['name'] 对上
            },
            {
                "action_type": "chat",
                "action": "Describe the generated meme to the user and provide the image path or URL.",
                "tool_use": []
            }
        ]
        return workflow
